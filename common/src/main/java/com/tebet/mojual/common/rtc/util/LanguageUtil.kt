...
class LanguageUtil {
    ...
    fun updateLanguage(context: Context) {
        ...
        if (currentLanguageIndex == PreferenceUtils.getInt(PREF_LANGUAGE_INDEX, -1)) return

        when (PreferenceUtils.getInt(PREF_LANGUAGE_INDEX, LANGUAGE_INDEX_ENGLISH)) {
    ...
    fun changeBahasa(context: Context) {
        PreferenceUtils.saveInt(PREF_LANGUAGE_INDEX, LANGUAGE_INDEX_BAHASA)
        changeLanguage(context, "in")
    }

    fun changeEnglish(context: Context) {
        PreferenceUtils.saveInt(PREF_LANGUAGE_INDEX, LANGUAGE_INDEX_ENGLISH)
