...
class AuthenticationInterceptor : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (!TextUtils.isEmpty(language) && !"in-ID".equals(language, ignoreCase = true) && !"in_ID".equals(
                language,
                ignoreCase = true
            )
        ) {
        ...
        if (currentToken != null && !TextUtils.isEmpty(currentToken.accessToken) &&
            ...
            chainRequest.addHeader(
                "Authorization",
                "Basic " + String(
                    Base64.encode(
                        String.format(
                            "%s:%s",
                            AuthSdk.instance.consumerKey,
                            AuthSdk.instance.consumerSecret
